...
dat$income <- as.factor(dat$p_gross_household)
dat$age_raw <- dat$age
...
pacman::p_load(tidyverse, haven, jtools, lme4, lmerTest, broom.mixed)

reg_fit <- lmer(immigSelf ~ private_renting +
                  male + 
                  white_british + white_other + indian + black + chinese + pakistan_bangladesh + mixed_race + 
                  no_religion + 
                  age + income + edu_20plus +
...
pca_fit <- lmer(immigSelf ~ private_renting +
                  male + 
                  white_british + white_other + indian + black + chinese + pakistan_bangladesh + mixed_race + 
                  no_religion + 
                  age + income + edu_20plus +
