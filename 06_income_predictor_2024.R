...
saveRDS(model_rf, file = "models/income_2024/model_rf_W29.RDS")
saveRDS(model_lm, file = "models/income_2024/model_lm_W29.RDS")
saveRDS(model_nn, file = "models/income_2024/model_nn_W29.RDS")
saveRDS(model_elm, file = "models/income_2024/model_elm_W29.RDS")
...
pred_df %>% 
  left_join(mod_dat %>% select(id, income), by = "id") %>% 
  ggplot(aes(x = pred_elm_round, y = income)) +
  geom_point(alpha = 0.25)

pred_df %>% 
  left_join(mod_dat %>% select(id, income), by = "id") %>% 
  select(income, pred_elm_round) %>% 
  corrr::correlate()

# saving ensemble model ------------------------------------------------

income_preds_W29 <- pred_df |> 
  rename(pred_el = pred_elm,
         pred_el_round = pred_elm_round) |> 
  select(id, pred_el, pred_el_round)

dat %>% 
  left_join(income_preds_W29, by = "id") %>% 
  ...
  theme_classic() +
  labs(title = "Imputed distribution of income BES Wave 29",
       caption = "Black = observed distribution; Red = imputed distribution",
       x = "Household income band")

ggsave("viz/imputed_income_W29.png")

saveRDS(income_preds_W29, file = "data/income_preds_W29.RDS")
...
saveRDS(knn_imp, file = "data/income_knn_W29.RDS")
